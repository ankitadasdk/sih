...
async def ingest_fir(
    ...
    try:
        ...
        # Normalize timeline items with dual-schema support (legacy + enriched)
        source_timeline = timeline_events[0] if timeline_events else case_timeline
        normalized_timeline = []

        for ev in source_timeline:
            ev_date = ev.get("datetime") or ev.get("date", "")
            ev_type = ev.get("category") or ev.get("title") or ev.get("type", "INCIDENT_LOG")
            ...
            normalized_timeline.append({
                ...
                # Enriched keys for UI tags & audit tracking
                "datetime": ev_date,
                "category": ev.get("category", ev_type),
                "title": ev.get("title", ev_type),
                "desc": ev_desc,
                "badgeColor": ev.get("badgeColor", "bg-rose-950 text-rose-300 border-rose-800"),
                "dotColor": ev.get("dotColor", "border-rose-500"),
                "citation": ev.get("citation", file.filename),
